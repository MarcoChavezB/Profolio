...
function connect(state, send, normalize) {
  ...
  function getItemProps(props) {
    ...
    return normalize.element({
      ...
      onPointerUp(event) {
        ...
        if (!isLeftClick(evt) || itemState.isDisabled)
          return;
        const target = event.currentTarget;
  ...
  return {
    ...
    getTriggerItemProps(childApi) {
      return mergeProps(getItemProps({ id: childApi.triggerProps.id }), childApi.triggerProps);
    },
    ...
    contentProps: normalize.element({
      ...
      onKeyDown(event) {
        const target = getEventTarget(getNativeEvent(event));
        const isKeyDownInside = target?.closest("[role=menu]") === event.currentTarget;
        if (!isSelfEvent(event) || !isKeyDownInside)
          return;
...
import { contains, getByTypeahead as getByTypeahead2, isEditableElement as isEditableElement2, raf } from "@zag-js/dom-query";
  ...
  return createMachine(
    ...
    },
    {
      ...
      guards: {
        ...
        isMenuFocused: (ctx2) => contains(dom.getContentEl(ctx2), dom.getActiveElement(ctx2)),
        ...
        isFocusedItemEditable: (ctx2) => isEditableElement2(dom.getFocusedItem(ctx2)),
