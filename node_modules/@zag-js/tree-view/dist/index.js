...
var dom = (0, import_dom_query.createScope)({
  getLabelId: (ctx) => `tree-label:${ctx.id}`,
  getRootId: (ctx) => `tree-root:${ctx.id}`,
  getTreeId: (ctx) => `tree-tree:${ctx.id}`,
  getNodeId(node) {
  ...
  getNodeEl(ctx, id) {
    ...
    if (node?.dataset.part === "branch") {
      return node.querySelector("[data-part=branch-control]");
    }
    return node;
  },
  getTreeEl(ctx) {
  ...
  return {
    ...
    treeProps: normalize.element({
      ...
      onKeyDown(event) {
        ...
        const target = (0, import_dom_query2.getEventTarget)(evt);
        ...
        const keyMap = {
          ArrowDown(event2) {
            if ((0, import_dom_event.isModifiedEvent)(event2))
              return;
          ...
          ArrowLeft(event2) {
            if ((0, import_dom_event.isModifiedEvent)(event2) || node.dataset.disabled)
              return;
            event2.preventDefault();
        ...
        if (exec) {
          ...
          const isValidTypeahead = event.key.length === 1 && !(0, import_dom_event.isModifiedEvent)(event);
          if (!isValidTypeahead)
            return;
    ...
    getItemProps(props) {
      ...
      return normalize.element({
        ...
        "data-focused": (0, import_dom_query2.dataAttr)(itemState.isFocused),
        role: "treeitem",
        "aria-current": itemState.isSelected ? "true" : void 0,
        "aria-selected": itemState.isDisabled ? void 0 : itemState.isSelected,
        "data-selected": (0, import_dom_query2.dataAttr)(itemState.isSelected),
    ...
    getBranchProps(props) {
      ...
      return normalize.element({
        ...
        "data-ownedby": dom.getTreeId(state.context),
        "aria-level": props.depth,
        "aria-selected": branchState.isDisabled ? void 0 : branchState.isSelected,
        "data-selected": (0, import_dom_query2.dataAttr)(branchState.isSelected),
        "aria-expanded": branchState.isExpanded,
        "data-state": branchState.isExpanded ? "open" : "closed",
        "aria-disabled": branchState.isDisabled,
        "data-disabled": (0, import_dom_query2.dataAttr)(branchState.isDisabled),
        style: {
          "--depth": props.depth
        }
      });
...
function machine(userContext) {
  ...
  return (0, import_core.createMachine)(
    ...
    {
      guards: {
        isBranchFocused: (ctx2, evt) => ctx2.focusedId === evt.id,
        isBranchExpanded: (ctx2, evt) => ctx2.expandedIds.has(evt.id),
        isShiftKey: (_ctx, evt) => evt.shiftKey,
        isCtrlKey: (_ctx, evt) => evt.ctrlKey,
        hasSelectedItems: (ctx2) => ctx2.selectedIds.size > 0,
      ...
      activities: {
        trackChildrenMutation(ctx2, _evt, { send }) {
          ...
          return (0, import_mutation_observer.observeChildren)(treeEl, (records) => {
            const removedNodes = records.flatMap((r) => Array.from(r.removedNodes)).filter((node) => {
              if (!(0, import_dom_query3.isHTMLElement)(node))
                return false;
            ...
            const nextSet = new Set(ctx2.selectedIds);
      ...
      actions: {
        setFocusableNode(ctx2) {
          if (ctx2.focusedId)
            return;
          if (ctx2.selectedIds.size > 0) {
            const firstSelectedId = Array.from(ctx2.selectedIds)[0];
            ctx2.focusedId = firstSelectedId;
            return;
          }
          const walker = dom.getTreeWalker(ctx2);
          const firstItem = walker.firstChild();
          if (!(0, import_dom_query3.isHTMLElement)(firstItem))
            return;
          ctx2.focusedId = dom.getNodeId(firstItem);
        },
        selectItem(ctx2, evt) {
          set.selected(ctx2, /* @__PURE__ */ new Set([evt.id]));
        },
        ...
        clearSelectedItem(ctx2) {
          set.selected(ctx2, /* @__PURE__ */ new Set());
        },
        toggleBranch(ctx2, evt) {
          const nextSet = new Set(ctx2.expandedIds);
          if (nextSet.has(evt.id)) {
            nextSet.delete(evt.id);
          } else {
            nextSet.add(evt.id);
          }
          set.expanded(ctx2, nextSet);
        },
        ...
        extendSelectionToItem(ctx2, evt) {
          ...
          const selectedIds = Array.from(ctx2.selectedIds);
        ...
        extendSelectionToNextItem(ctx2, evt) {
          ...
          const nextNode = walker.nextNode();
          dom.focusNode(nextNode);
          const selectedIds = new Set(ctx2.selectedIds);
        ...
        extendSelectionToFirstItem(ctx2) {
          const nodes = dom.getTreeNodes(ctx2);
          const anchorEl = dom.getNodeEl(ctx2, [...ctx2.selectedIds][0]) || nodes[0];
